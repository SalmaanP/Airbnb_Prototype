...
exports.getAllHosts = function (req, res, next) {
    ...
    User.find({isHost: true, isApproved: true, isActivated: true}, function (err, results) {
        if (err) {
            throw err;
        }
        else {
...
exports.unapprovedHost = function (req, res) {
    User.find({isHost: true, isApproved: false, isActivated: true}, function (err, results) {
        if (err) {
            throw err;
        } else {
            console.log(results);
